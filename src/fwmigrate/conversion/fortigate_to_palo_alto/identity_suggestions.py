...
def build_identity_recommendations(source, derived, decisions, target=None, target_device=None):
    ...
    for user in getattr(source, "local_users", ()):
        ...
        candidates = target_names(target, "local_users", target_device)
        ...
        result.append(PANMigrationRecommendation(
            ...
            PANRecommendationMethod.TARGET_EVIDENCE if name in candidates else PANRecommendationMethod.DETERMINISTIC,
            PANRecommendationConfidence.HIGH, evidence, candidates, (), tuple(blockers), ()))
    for group in getattr(source, "user_groups", ()):
        ...
        result.append(PANMigrationRecommendation(
            ...
            target_names(target, "local_user_groups", target_device), (), blockers, ()))
    return tuple(result)
