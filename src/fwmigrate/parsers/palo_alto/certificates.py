...
def extract_certificates(scope: PANScope, root: ET.Element, extraction, resolver) -> None:
    ...
    for entry in root.findall("./ssl-tls-service-profile/entry"):
        ...
        if not name:
            ...
            continue
        certificate = text_or_none(entry, "./certificate")
        certificate_profile = text_or_none(entry, "./certificate-profile")
        item = IRSSLTLSServiceProfile(name=name, source_context=f"{scope.kind}:{scope.name}",
            certificate=certificate, certificate_profile=certificate_profile,
