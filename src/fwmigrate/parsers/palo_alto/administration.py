...
from fwmigrate.ir.core import IRAdministrator
...
def extract_administrators(root: ET.Element, extraction) -> None:
    ...
    for entry in users:
        ...
        item = IRAdministrator(name=name, access_profile=role or profile,
            credential_configured=entry.find("./phash") is not None,
            authentication_profile=auth, authentication_sequence=sequence,
            remote_auth=auth, source_attributes=sanitize_source_attributes(source),
