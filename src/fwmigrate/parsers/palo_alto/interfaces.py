...
def _sanitize_attrs(attrs: Dict[str, Any]) -> Dict[str, Any]:
    """Sanitize structured PAN-OS source evidence before it leaves the parser."""
...
def _ipv6_neighbor_discovery(node: ET.Element) -> Dict[str, Any]:
    """Project PAN-OS IPv6 ND/RA fields while retaining the complete source tree."""
    result: Dict[str, Any] = {"source_entry": structured_xml_capture(node)}
    for field in (
        "enable-ndp-monitor",
        "enable-dad",
...
def _ipv6_dhcp_client(node: ET.Element) -> Dict[str, Any]:
    """Extract PAN-OS DHCPv6 client semantics without flattening nested choices."""
    result: Dict[str, Any] = {"source_entry": structured_xml_capture(node)}
    ...
    if options is not None:
        ...
        for path, key in (
            ...
            ("./duid-type", "duid_type"),
            ("./enable/yes/non-temp-addr", "non_temp_addr"),
            ("./enable/yes/temp-addr", "temp_addr"),
        ):
    ...
    if delegation is not None:
        ...
        for path, key in (
            ("./enable", "enable"),
            ("./enable/yes/prefix-len-hint", "prefix_len_hint"),
            ("./enable/yes/prefix-len", "prefix_len"),
            ("./enable/yes/pfx-pool-name", "prefix_pool_name"),
        ):
...
def parse_layer3_interface(
    ...
    for child_name, attribute_name in (
        ("bonjour", "pan_bonjour"),
        ("sdwan-link-settings", "pan_sdwan_link_settings"),
        ("ddns-config", "pan_ddns_config"),
        ("arp", "pan_arp"),
    ):
    ...
    if dhcp is not None:
        ...
        attrs["pan_dhcp_client_fields"] = {
            ...
            for key, value in {
                "enable": dhcp_enabled,
                "create_default_route": text_or_none(dhcp, "./create-default-route"),
    ...
    if pppoe is not None:
        ...
        attrs["pan_pppoe_fields"] = {
            key: value
            for key, value in {
                "enable": pppoe_enabled,
                "authentication": text_or_none(pppoe, "./authentication"),
                ...
                "service": text_or_none(pppoe, "./service"),
                "passive_enable": text_or_none(pppoe, "./passive/enable"),
            }.items()
            if value is not None
        }
    ...
    known_layer3 = [
        "ip", "ipv6", "comment", "interface-management-profile", "dhcp-client", "pppoe", "tag",
        "units", "link-state", "mtu", "adjust-tcp-mss", "ndp-proxy", "lldp", "netflow-profile",
        "decrypt-forward", "cluster-interconnect", "traffic-interconnect", "untagged-sub-interface",
        "bonjour", "sdwan-link-settings", "ddns-config", "arp",
    ]
    ...
    if physical_node is not None:
        physical_unknown = collect_unknown_children(
            physical_node,
            [
                *MODES, "comment", "link-state", "link-speed", "link-duplex", "speed", "duplex", "mtu",
                "lldp", "aggregate-group", "fec", "poe", "lacp",
            ],
        )
        if physical_unknown:
    ...
    interface = IRInterface(
        ...
        source_ipv6_send_adv=ra_local.get("enable"),
        source_ipv6_manage_flag=ra_local.get("managed_flag"),
        source_ipv6_other_flag=ra_local.get("other_flag"),
        ...
        dhcp_client=(dhcp_enabled.lower() == "yes") if dhcp_enabled is not None else None,
...
def parse_layer2_interface(
    ...
    lldp = node.find("./lldp")
    attrs.update({
        "pan_interface_mode": "layer2",
        "pan_parent_interface": parent,
        "pan_vlan_tag": text_or_none(node, "./tag"),
        "pan_layer2_netflow_profile": netflow,
    ...
    unknown = collect_unknown_children(
        node,
        ["comment", "interface-management-profile", "tag", "units", "lldp", "mtu", "netflow-profile"],
    )
    if unknown:
...
def parse_virtual_wire_interface(
    node: ET.Element,
    name: str,
    parent: Optional[str] = None,
    physical_node: Optional[ET.Element] = None,
    ...
    lacp = node.find("./lacp")
    attrs.update({
        "pan_interface_mode": "virtual-wire",
        "pan_parent_interface": parent,
...
def parse_virtual_wire_subinterface(node: ET.Element, name: str, parent: str) -> Dict[str, Any]:
    ...
    if not classifiers:
        ...
        if scalar:
            classifiers = [scalar]
    attrs.update({
        "pan_interface_mode": "virtual-wire-subinterface",
        "pan_parent_interface": parent,
        "pan_vlan_tag": text_or_none(node, "./tag"),
        "pan_virtual_wire_netflow_profile": text_or_none(node, "./netflow-profile"),
...
def parse_ha_interface(
    ...
    lacp = node.find("./lacp")
    attrs.update({
        "pan_interface_mode": "ha",
        "pan_parent_interface": parent,
        "pan_ha_settings": _mode_semantics(node),
    })
    if lacp is not None:
...
def parse_decrypt_mirror_interface(
    node: ET.Element,
    name: str,
    parent: Optional[str] = None,
    physical_node: Optional[ET.Element] = None,
    ...
    attrs = _source_fields(node, physical_node)
    attrs.update({
        "pan_interface_mode": "decrypt-mirror",
        "pan_parent_interface": parent,
...
def _issues(attrs: Dict[str, Any]) -> list[str]:
    ...
    source_only_physical = {
        "pan_aggregate_group": "Aggregate-group semantics remain source-oriented.",
    ...
    if (
        ...
        or attrs.get("pan_sdwan_unknown_fields")
    ):
...
def _build_source_interface(
    ...
    *,
    ...
    netflow = (
        attrs.get("pan_netflow_profile")
        or attrs.get("pan_layer2_netflow_profile")
        or attrs.get("pan_virtual_wire_netflow_profile")
        or attrs.get("pan_tap_netflow_profile")
    )
    lldp_enabled = (
        attrs.get("pan_layer2_lldp_enabled")
        or attrs.get("pan_virtual_wire_lldp_enabled")
        or attrs.get("pan_lldp_enabled")
    )
    ...
    return IRInterface(
        ...
        source_speed=attrs.get("pan_speed"),
        source_duplex=attrs.get("pan_duplex"),
        source_netflow_profile=netflow,
        source_lldp_enabled=lldp_enabled,
        source_attributes=attrs,
...
def _annotate_network_interface_relationships(network_root: ET.Element, scope: PANScope, ir, extraction) -> None:
    """Attach VLAN and virtual-wire relationships defined outside interface entries."""
        ...
        if virtual_interface:
            for interface in _interfaces_in_scope(ir, scope, virtual_interface):
                ...
                _update_interface_inventory(
                    ...
                    virtual_interface,
                    {
                        "pan_vlan_virtual_interface_for": list(
                            interface.source_attributes["pan_vlan_virtual_interface_for"]
                        )
                    },
                )
...
def extract_interfaces(network_root: ET.Element, scope: PANScope, ir, resolver, extraction) -> None:
    ...
    physical_parsers = {
        "ethernet": parse_ethernet_interface,
        "aggregate-ethernet": parse_aggregate_ethernet_interface,
    }
    mode_parsers = {
        "layer2": parse_layer2_interface,
        "virtual-wire": parse_virtual_wire_interface,
        "tap": parse_tap_interface,
        "ha": parse_ha_interface,
    ...
    logical = {
        "loopback": parse_loopback_interface,
        "tunnel": parse_tunnel_interface,
    ...
    for family, parser in logical.items():
        ...
        for entry in entries:
            ...
            path = (
                f"network/interface/{family}/units/entry[@name='{name}']"
                if is_unit
    ...
    if sdwan_root is not None:
        for entry in sdwan_root.findall("./units/entry"):
            ...
            attrs = {
                ...
                "pan_sdwan_unknown_fields": unknown or None,
    ...
    for instance, members in discovered_instances:
        ...
        evidence.update({
            ...
            "pan_source_path": (
                ...
                else "network/routing-instance/interface/member"
            ),
        })
        record_partial(
            extraction,
