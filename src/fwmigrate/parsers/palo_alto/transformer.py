...
from .parser import _PANOSBaseSourceParser


class PANToIRTransformer(_PANOSBaseSourceParser):
    """Construct canonical PAN-OS IR from the parsed source document."""
    ...
    def transform(self, content: str, zone_mapping: Optional[Dict[str, str]] = None) -> ExtractionResult:
        ...
        source = self.parse_source(content)
        root = source.root
    ...
    def extract(self, content: str, zone_mapping: Optional[Dict[str, str]] = None):
        """Compatibility entry point for legacy transformer subclasses."""
        return self.transform(content, zone_mapping)



__all__ = ["PANToIRTransformer"]
