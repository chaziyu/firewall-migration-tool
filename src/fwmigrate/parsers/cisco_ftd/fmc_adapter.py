...
class CiscoFMCBundleParser(_RawCiscoFMCBundleParser):
    ...
    def parse(self) -> IRConfig:
        # Reuse the raw parser's typed section methods, but build IRMetadata only
        # from fields that exist in the canonical schema.  FMC-specific evidence
        # lives on the normalized objects/rules and generation-blocking reasons.
        ...
        for policy in ir.policies:
            ...
            # service.  Preserve it explicitly until canonical IR grows a
            # source-port policy criterion.
            ...
            if source_ports:
                ...
                reason = "FMC source-port match is source-preserved; canonical policy IR has no independent source-port criterion"
            ...
            if policy.source_users or policy.source_user_groups:
                ...
                reason = "FMC user/identity criteria require target identity-provider validation"
                if reason not in policy.review_reasons:
                    policy.review_reasons.append(reason)
                policy.requires_manual_review = True
                policy.migration_status = "PARTIALLY_NORMALIZED"
        ...
        for rule in ir.nat_rules:
            ...
            # DYNAMIC_IP is now represented exactly in canonical IR, but not all
            # target generators advertise address-only dynamic NAT support yet.
            # Preserve the exact mode and withhold generation rather than letting
            # a target silently emit PAT or no translation.
            if rule.source_translation_mode == NATTranslationMode.DYNAMIC_IP:
                reason = "Address-only dynamic NAT is canonicalized, but target-generator support must be validated"
