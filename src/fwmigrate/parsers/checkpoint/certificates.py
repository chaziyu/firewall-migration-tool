...
def _identity(domain: str, obj: Dict[str, Any], cert: IRCertificate) -> tuple[str, ...]:
    if cert.source_uid:
        return domain, "uid", cert.source_uid
    if cert.sha256_fingerprint:
        return domain, "fingerprint", cert.sha256_fingerprint.lower()
    if cert.issuer and cert.serial_number:
        return domain, "issuer-serial", cert.issuer, cert.serial_number
    return domain, "subject-validity", cert.subject or "", str(cert.valid_from), str(cert.valid_until)
...
def _usage_from_object(obj: Dict[str, Any], consumer_type: str, consumer_uid: Any, consumer_name: Any, path: str) -> List[dict]:
    uses = []
    for key, value in obj.items():
        if str(key).lower().replace("_", "-") == "sic" and isinstance(value, dict):
            ...
            if _is_certificate_reference(cert_value):
                ...
                uses.append({"certificate_uid": uid, "certificate_name": name, "consumer_type": consumer_type,
...
def extract_certificates(responses: Iterable[Any], usage_sources: Iterable[Any] = ()) -> Tuple[List[IRCertificate], List[SourceInventoryItem]]:
    ...
    for response in responses:
        ...
        for index, obj in enumerate(_objects(response)):
            ...
            if family == "gateway":
                all_usage.extend(_usage_from_object(obj, "gateway", obj.get("uid"), obj.get("name"), path))
            ...
            cert = IRCertificate(name=name, certificate_type=str(obj.get("type") or "checkpoint"), source_uid=uid,
                ...
                source_attributes={**_safe_attributes(obj), "source_command": getattr(response, "command", None), "source_path": path})
            identity = _identity(domain, obj, cert)
            existing = by_identity.get(identity)
            if existing is None:
                by_identity[identity] = cert
                result.append(cert)
                existing = cert
            all_usage.extend(_usage_from_object(obj, "certificate", uid, name, path))
    ...
    for source in usage_sources:
        ...
        for record in source:
            ...
            for reference in refs:
                ...
                all_usage.append({"certificate_uid": uid, "certificate_name": name, "consumer_type": "vpn",
    ...
    for use in all_usage:
        ...
        if not matches and use.get("certificate_name"):
            matches = [cert for cert in result if cert.name == use["certificate_name"]]
        for cert in matches:
        ...
        if not matches:
            inventory.append(SourceInventoryItem(
                domain="global", source_path=str(use.get("source_path") or "checkpoint/certificate-reference"),
    ...
    for source in sources:
        ...
        for record in source:
            ...
            if not reference:
                continue
            uid, name = _reference(reference)
            matches = [item for item in certificates if uid and item.source_uid == uid]
            if not matches and name:
                matches = [item for item in certificates if item.name == name]
            usage = "HTTPS_INSPECTION_CA" if "certificate" in attrs and "source_uuid" in attrs else "IKE"
            relation = {"certificate_uid": uid, "certificate_name": name, "consumer_type": "https-inspection" if usage.startswith("HTTPS") else "vpn",
