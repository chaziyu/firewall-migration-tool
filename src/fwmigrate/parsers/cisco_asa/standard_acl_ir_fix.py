...
def apply_standard_acl_ir_fixes(ir: Any, config: Any) -> Any:
    """Apply standard-ACL projection to an already-built canonical IR."""
    standard_rules = {
    ...
    for policy in ir.policies:
        ...
        policy.review_reasons = [
            reason for reason in policy.review_reasons if reason not in obsolete
        ]
...
def apply_standard_acl_ir_fix(parser_cls: Any) -> None:
    ...
    def transform(self: Any):
        return apply_standard_acl_ir_fixes(original(self), self.config)
