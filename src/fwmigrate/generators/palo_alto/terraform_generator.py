...
"""
    ...
    def _generate_main_tf(self, ir: IRConfig, ordered: dict) -> str:
        ...
        self.ipv4_helper_name, self.ipv4_reused = allocate_target_helper_name(
        ...
        for p in ir.policies:
            if not p.safe_for_target_generation or policy_references_unsafe_zone(p, unsafe_zones):
                continue
            for s in (p.source or []):
                fam = classify_universal_address_reference(s)
                if fam == AddressUniversalFamily.IPV4:
                    self.needed_helpers.add("ipv4")
...
"""
    ...
    def _generate_address_objects(self, ir: IRConfig, addresses: list) -> str:
        ...
        if "ipv4" in self.needed_helpers and not self.ipv4_reused:
            tf_name = self.sanitize_tf_name(self.ipv4_helper_name)
            self.generated_addresses[self.ipv4_helper_name] = tf_name
            panos_name = self.sanitize_panos_name(self.ipv4_helper_name)
            output.append(f"""resource "panos_address_object" "{tf_name}" {{
  vsys        = var.panos_vsys
  name        = "{panos_name}"
  value       = "0.0.0.0/0"
  type        = "ip-netmask"
  description = "Universal IPv4 helper generated by firewall-migration-tool"
}}
""")
        ...
        for addr in addresses:
            ...
            if addr.type == AddressType.STUB_UNSUPPORTED:
                output.append(f"# SKIPPED Address '{addr.name}': unsupported source address semantics require manual review\n")
                continue
...
""")
                ...
                output.append(f"# SKIPPED Address '{addr.name}': unsupported source address type '{addr.type.value}' requires manual review\n")
                ir.audit_entries.append(IRAuditEntry(
                    id=f"panos-address-unsupported:{addr.name}",
                    category="PAN-OS Address",
                    message=f"Address '{addr.name}' of type '{addr.type.value}' is unsupported and was withheld from PAN-OS generation.",
    ...
    def _generate_security_policies(self, ir: IRConfig, policies: list) -> str:
        ...
        for p in policies:
            ...
            if p.source and "all" not in p.source:
                for a in p.source:
                    fam = classify_universal_address_reference(a)
                    if fam == AddressUniversalFamily.IPV6:
                        ir.audit_entries.append(IRAuditEntry(
                            id=f"panos-terraform-policy-ipv6:{p.source_rule_id or p.name}",
                            category="PAN-OS Terraform Policy",
                            message=f"Policy '{p.name}' contains IPv6 any-references which require manual review in PAN-OS Terraform. Policy withheld.",
                            confidence=MigrationConfidence.MANUAL,
                        ))
                        skip_policy = True
                        break
                    elif fam == AddressUniversalFamily.IPV4:
                        valid_source_addrs.append(self.sanitize_panos_name(self.ipv4_helper_name))
                    elif fam == AddressUniversalFamily.ANY:
                        valid_source_addrs.append("any")
                    elif a in self.generated_addresses or a in self.generated_address_groups or a in self.generated_url_categories:
            ...
            if p.destination and "all" not in p.destination:
                for a in p.destination:
                    ...
                    if fam == AddressUniversalFamily.IPV6:
                        ...
                        valid_dest_addrs.append(self.sanitize_panos_name(self.ipv4_helper_name))
                    elif fam == AddressUniversalFamily.ANY:
            ...
            source_addrs = ["any"] if "any" in valid_source_addrs else (valid_source_addrs or [])
            dest_addrs = ["any"] if "any" in valid_dest_addrs else (valid_dest_addrs or [])
