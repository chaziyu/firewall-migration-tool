...
class CheckPointCLIGenerator:
    ...
    def generate(self, ir: IRConfig) -> str:
        ...
        existing_names = {a.name for a in ir.addresses}
        ipv4_helper_name = "__fwmigrate_any_ipv4"
        if ipv4_helper_name in existing_names:
            addr_obj = next((a for a in ir.addresses if a.name == ipv4_helper_name), None)
            if not addr_obj or addr_obj.value != "0.0.0.0/0":
                ipv4_helper_name = "__fwmigrate_internal_any_ipv4"

        ipv6_helper_name = "__fwmigrate_any_ipv6"
        if ipv6_helper_name in existing_names:
            addr_obj = next((a for a in ir.addresses if a.name == ipv6_helper_name), None)
            if not addr_obj or addr_obj.value != "::/0":
                ipv6_helper_name = "__fwmigrate_internal_any_ipv6"
