...
def transform_nat(rules: list[PANNATRule]) -> tuple[PANDerivedNAT, ...]:
    ...
    for rule in rules:
        ...
        result.append(PANDerivedNAT(rule.name, rule.source_path, type(source).__name__ if source else None, type(destination).__name__ if destination else None, dynamic.distribution if dynamic else None, refs, rule.source_order))
    return tuple(result)
