...
def extract_schedule(element: ET.Element, path: tuple[str, ...], context: PANWalkContext, source_order: int) -> object | None:
    if path[-2:] != ("schedule", "entry"):
        return None
    extra, explicit = typed_fields(element, {"schedule-type"})
