...
from ..model import PANDestinationTranslation, PANDNSRewrite, PANDynamicDestinationTranslation, PANDynamicIPAndPortTranslation, PANDynamicIPTranslation, PANStaticIPTranslation, PANNATRule
...
def _source(element: ET.Element):
    ...
    if node is None:
        return None
    for branch in ("dynamic-ip-and-port", "persistent-dynamic-ip-and-port", "dynamic-ip"):
        dynamic = node.find(branch)
        if dynamic is None:
            continue
        interface = dynamic.find("interface-address")
        known = {"translated-address", "interface-address", "fallback"} if branch == "dynamic-ip" else {"translated-address", "interface-address"}
        extra, explicit = typed_fields(dynamic, known, {"translated-address": "translated_addresses", "fallback": "fallback"})
        explicit.discard("interface-address")
        explicit.add("translation_type")
        ...
        fields = dict(translation_type=branch, translated_addresses=values(dynamic, "translated-address"), interface=value(interface, "interface"), ip=value(interface, "ip"), raw_extra=extra, explicit_fields=explicit)
        if branch == "dynamic-ip":
            fallback = dynamic.find("fallback")
            fields["fallback"] = sanitize_source_attributes(structured_xml_capture(fallback)) if fallback is not None else None
            return PANDynamicIPTranslation(**fields)
        return PANDynamicIPAndPortTranslation(**fields)
    ...
    if static is not None:
        ...
        explicit.add("translation_type")
        return PANStaticIPTranslation(translation_type="static-ip", translated_address=value(static, "translated-address"), bi_directional=value(static, "bi-directional"), raw_extra=extra, explicit_fields=explicit)
    return None
