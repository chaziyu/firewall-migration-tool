...
def classify_cisco_asa_coverage(sections: list[SourceSectionResult]) -> None:
    for section in sections:
        if section.path in EXTRACTED:
            ...
            section.parser_handler = "CiscoASAParser.parse_raw"
            section.notes.append("Portable semantics are extracted where proven; source-only details remain for review.")
        ...
        elif section.path in SOURCE_ONLY:
            section.status = ExtractionStatus.SOURCE_ONLY
