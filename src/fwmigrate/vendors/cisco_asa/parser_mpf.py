...
def _parse_class_map_header(parts: List[str], line_number: int) -> Tuple[str, Optional[str], Optional[str], Optional[str], bool]:
    ...
    if len(parts) >= 4 and [part.lower() for part in parts[:2]] == ["class-map", "type"]:
...
def _parse_class_map_block(self: Any, lines: List[str], index: int) -> CiscoClassMap:
...
def _parse_policy_map_header(parts: List[str], line_number: int) -> Tuple[str, Optional[str], Optional[str], bool]:
    ...
    if len(parts) == 5 and [part.lower() for part in parts[:3]] == ["policy-map", "type", "inspect"]:
        return parts[4], "inspect", parts[3].lower(), False
    return _header_error_name("policy-map", line_number), None, None, True


def _parse_policy_map_block(self: Any, lines: List[str], index: int) -> CiscoPolicyMap:
    line = lines[index].strip()
    parts = line.split()
...
def _parse_mpf_action(self: Any, section: CiscoPolicyMapClass, line: str, line_number: int) -> None:
...
def _parse_connection_action(self: Any, section: Any, line: str, line_number: int) -> CiscoMPFConnectionAction:
    ...
    if negated:
        parts = parts[1:]
    action = CiscoMPFConnectionAction(raw=safe, source_order=line_number, negated=negated)
...
def _parse_police_action(self: Any, section: Any, line: str, line_number: int) -> CiscoMPFPoliceAction:
    safe = sanitize_raw_text(line)
    parts = line.split()
    negated = parts[:1] == ["no"]
    if negated:
        parts = parts[1:]
    action = CiscoMPFPoliceAction(raw=safe, source_order=line_number, negated=negated)
...
def _parse_tcp_map_block(self: Any, lines: List[str], index: int) -> CiscoTCPMap:
    line = lines[index].strip()
    parts = line.split()
...
def _parse_service_policy_line(self: Any, line: str, line_number: int) -> CiscoServicePolicy:
    safe = sanitize_raw_text(line)
    parts = line.split()
    negated = parts[:1] == ["no"]
    if negated:
...
def _parse_threat_detection(self: Any, line: str, line_number: int) -> CiscoConnectionControl:
    parts = line.split()
    negated = parts[:1] == ["no"]
    if negated:
        parts = parts[1:]
    ...
    if item.threat_detection_type == "rate":
        allowed = {"average-rate", "burst-rate", "interval"}
        pos = 0
        while pos < len(params):
            ...
            if not value.isdigit():
                ...
                break
            numeric = int(value)
            if key == "average-rate": item.average_rate = item.rate = numeric
            elif key == "burst-rate": item.burst_rate = item.burst = numeric
            else: item.source_attributes["interval"] = numeric
            pos += 2
        return item
...
def _parse_dhcpd_command(self: Any, line: str, line_number: int) -> None:
    safe = sanitize_raw_text(line)
    parts = line.split()
    negated = parts[:1] == ["no"]
    if negated:
...
def _parse_management_command(self: Any, line: str, line_number: int) -> None:
    ...
    if command in {"ssh", "http", "telnet"} and len(effective_parts) >= 3:
        ...
        if effective_parts[1].lower() in {"timeout", "version", "key-exchange", "cipher", "authentication"}:
            return self._parse_management_command_base(line, line_number)
...
def _parse_trustpoint_block(self: Any, lines: List[str], index: int) -> int:
    line = lines[index].strip()
    match = re.fullmatch(r"crypto\s+ca\s+trustpoint\s+(\S+)", line, re.I)
    if not match:
        return index + 1
    record = CiscoTrustpointRecord(
        name=match.group(1), source_context=self._line_contexts.get(index + 1), source_order=index + 1,
        raw_lines=[sanitize_raw_text(line)], source_attributes={"raw_command": sanitize_raw_text(line)},
    )
    cursor = index + 1
    while cursor < len(lines) and lines[cursor][:1].isspace() and lines[cursor].strip() and not lines[cursor].strip().startswith("!"):
        ...
        if key in {"password", "key", "pin", "private-key"}:
            ...
            record.raw_lines.append(safe)
            values = tokens[1:]
            if key == "enrollment": record.enrollment = " ".join(values)
            elif key == "subject-name": record.subject_name = " ".join(values)
            elif key in {"keypair", "keypair-reference"} and values: record.keypair_reference = values[0]
            elif key == "revocation-check": record.revocation_check = values
    ...
    if record.name not in self.config.trustpoints:
        self.config.trustpoints.append(record.name)
    return cursor
