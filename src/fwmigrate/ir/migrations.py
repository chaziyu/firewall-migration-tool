...
def _migrate_1_52_to_1_61(payload: dict[str, Any]) -> dict[str, Any]:
    return migrate_1_60_to_1_61(
...
def migrate_ir_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = _normalize_ssl_vpn_ciphersuite(payload)
    if version == "1.52":
        return _migrate_1_52_to_1_61(payload)
    if version == "1.53":
        return _migrate_1_52_to_1_61({**payload, "schema_version": "1.52"})
    if version == "1.54":
        return _migrate_1_52_to_1_61({**payload, "schema_version": "1.52"})
    if version == "1.55":
        return _migrate_1_52_to_1_61({**payload, "schema_version": "1.52"})
    ...
    if version == "1.57":
        return _migrate_1_52_to_1_61({**payload, "schema_version": "1.52"})
    if version == "1.58":
        return migrate_1_60_to_1_61(migrate_1_59_to_1_60(migrate_1_58_to_1_59(dict(payload))))
    if version == "1.59":
        return migrate_1_60_to_1_61(migrate_1_59_to_1_60(dict(payload)))
    if version == "1.60":
        return migrate_1_60_to_1_61(dict(payload))
    if version == "1.51":
