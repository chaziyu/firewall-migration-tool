...
class PANSSHDeployer:
    ...
    def push_candidate(self, commands):
        results = []
        try:
    ...
    def commit_candidate(self):
        ...
        match = _JOB.search(response or "")
        return PANCommitResult(match.group(1) if match else None, "SUCCESS", response or "")

    def deploy(self, commands):
        self.connect()
        try:
            pushed = self.push_candidate(commands)
            if pushed.failed_command_index is not None:
                return pushed
...
def deploy_set_commands(commands, options: PANDeploymentOptions) -> PANDeploymentResult:
    """Compatibility wrapper; commit remains opt-in through options.commit."""
    return PANSSHDeployer(options).deploy(commands)
