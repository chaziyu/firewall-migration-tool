...
class IRToPANOSTransformer:
    ...
    def transform(self) -> PANConfig:
        ...
        for s in self.ir.services:
            ...
            # (complex multi-port services might need multiple entries)
            if s.ports:
                port = s.ports[0]
                ...
                # Note: ICMP is handled differently in PAN-OS policies directly, usually not as a service object
            
            # If no TCP/UDP, we skip creating a service object for MVP (or handle it in rule)
        ...
        # 6. Transform Policies (Security Rules)
        for p in self.ir.policies:
            ...
            pan.vsys.security_rules.append(PANRuleEntry(
                ...
                service=p.service,
                action=action_map.get(p.action, "deny"),
        ...
        for n in self.ir.nat_rules:
            rule = PANNATRuleEntry(
                name=n.name,
                from_zones=n.from_zone or ["any"],
                to_zones=n.to_zone or ["any"],
                source=n.source or ["any"],
                destination=n.destination or ["any"],
                service=n.service
            )
            if n.type == NATType.SOURCE and n.translated_source:
                rule.source_translation = n.translated_source
            elif n.type == NATType.DESTINATION and n.translated_destination:
                rule.destination_translation = n.translated_destination
                
            pan.vsys.nat_rules.append(rule)
