...
from fg2pan.transformer.ir_to_panos import IRToPANOSTransformer
from fg2pan.generator.panos_model import PANConfig
            ...
            if config.vsys.security_rules:
                ...
                for r in config.vsys.security_rules:
                    ...
                    for field in ["to", "from", "source", "destination", "source-user", "category", "application", "service", "source-hip", "destination-hip"]:
                        # map py name to xml name
                        py_name = field.replace("-", "_")
                        # "to" and "from" are python keywords, we mapped them to to_zones and from_zones
