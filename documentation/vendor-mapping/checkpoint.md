...
| Policy package | Access policy package | package UID, layers, installation targets | `IRCheckpointPolicyPackage` | package identity and associations | Direct | Full | `extractor.py` |
| Access layer | Access Control layer | layer UID, parent layer/rule, rule order | `IRCheckpointAccessLayer` | layer identity and scoped rule order | Direct | Full | `extractor.py`, `rulebase.py` |
| Domain/global assignment | Multi-Domain / global policy | domain UID, assignment, package mapping | `IRCheckpointDomain`, `IRCheckpointGlobalAssignment` | domain and assignment fields | Semantic | Partial | `extractor.py`, `gaia_scoped.py` |
