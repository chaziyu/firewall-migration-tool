...
| NAT / VIP / IP pools | `NORMALIZED` / `PARTIALLY_NORMALIZED` / `EXTRACT_ONLY` | Broad | Conditional | `central-snat-map`, `ippool`, `ippool6`, `vip`, and `vipgrp` are FortiOS source constructs. Basic IPv4 SNAT/DNAT is normalized where semantics are proven. Central SNAT ambiguity, advanced pools/VIPs, unresolved VIP-group members, and IPv6 pool semantics remain review-gated or extract-only. |
