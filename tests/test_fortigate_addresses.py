...
def test_fortigate_address_excel_exposes_source_metadata():
    ...
    ems_row = row_by_name["ems"]
    assert sheet.cell(ems_row, headers["Type"]).value == "dynamic"
    assert sheet.cell(ems_row, headers["Source Section"]).value == "firewall address"
    assert sheet.cell(ems_row, headers["Source Type"]).value == "dynamic"
    assert sheet.cell(ems_row, headers["EMS Sub-Type"]).value == "ems-tag"
    assert sheet.cell(ems_row, headers["EMS Object Tag"]).value == "Deleum_ADUser"
    ...
    assert "ems" not in group_row_by_name
