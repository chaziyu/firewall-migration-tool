...
from fwmigrate.ir.core import IRAddress, IRConfig, IRMetadata, IRPolicy, IRService
...
def test_capability_conservation_invariant():
    ...
    svc = IRService(name="svc1", migration_status="EXTRACT_ONLY", requires_manual_review=True)
    addr = IRAddress(name="addr1", type=AddressType.NETWORK, subnet="10.0.0.0/8", migration_status="PARTIALLY_NORMALIZED")
    ir = IRConfig(metadata=IRMetadata(source_vendor="test"), policies=[rule], services=[svc], addresses=[addr])
