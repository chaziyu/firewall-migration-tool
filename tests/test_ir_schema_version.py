...
def test_schema_1_47_aggregate_interface_migrates_without_inventing_source_fields():
...
def test_schema_1_62_migrates_to_current_without_inventing_palo_alto_values():
    migrated = migrate_ir_payload({
        "schema_version": "1.62",
        "metadata": {"hostname": "legacy-fw", "source_vendor": "palo_alto"},
    })
...
@pytest.mark.parametrize("value", ["0.9", "1.64", "2.0"])
...
def test_schema_1_41_sdwan_member_additive_fields_use_safe_defaults():
    ...
    member = ir.sdwan.members[0]
    assert member.preferred_source is None
    assert member.transport_group is None
    assert member.review_reasons == []
    assert member.source_explicit_fields == []
