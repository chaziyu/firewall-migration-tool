...
def test_ssl_tls_service_profile_does_not_treat_certificate_profile_as_certificate():
    ...
    assert profile.certificate_profile == "client-cert-profile"
...
def test_ssl_tls_service_profile_reports_unresolved_references_independently():
    result = _extract(
        '<ssl-tls-service-profile><entry name="tls-profile">'
        '<certificate>missing-server-cert</certificate>'
        '<certificate-profile>missing-client-cert-profile</certificate-profile>'
        '</entry></ssl-tls-service-profile>'
    )

    profile = result.canonical_ir.ssl_tls_service_profiles[0]

    assert profile.certificate == "missing-server-cert"
    assert profile.certificate_resolved is False
    assert profile.certificate_profile == "missing-client-cert-profile"
    assert profile.certificate_profile_resolved is False
    assert "unresolved-certificate-reference" in profile.review_reasons
    assert "unexpected-certificate-profile-reference" in profile.review_reasons
    assert "unresolved-certificate-profile-reference" in profile.review_reasons
